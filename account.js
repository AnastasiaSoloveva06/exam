...
async function fetchOrders() {
    ...
    // Сортируем: те, у кого ID больше (новые), идут первыми
...
function openEditModal(id) {
    ...
    // Находим объект курса или репетитора, к которому относится заявка
    ...
    // Определяем доступные даты (логика как в main.js)
    let availableDates = [];
    if (o.course_id) {
        availableDates = item ? (item.start_dates || []) : [];
    } else {
        // Для репетиторов (заглушка дат из вашего main.js)
    ...
    // Если текущей даты заявки нет в списке доступных (например, курс обновился), 
    // добавляем её, чтобы форма не была пустой
...
function updateEditPrice() {
    ...
    // 6. Дополнительные чекбоксы (те, которые можно менять)
...
// Привязываем пересчет цены к изменениям полей
...
document.getElementById('edit-form').onsubmit = async (e) => {
    ...
    // Извлекаем "чистое" число из текста, удаляя все пробелы (в том числе неразрывные)
