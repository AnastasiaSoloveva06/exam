...
// Яндекс.Карты глобальные переменные
let myMap;
let objectManager;
...
// --- ОСТАЛЬНАЯ ЛОГИКА (РЕНДЕР И РАСЧЕТЫ) ---
...
function renderTutors() {
    ...
    tutorsContainer.innerHTML = filtered.map(t => {
        // Проверяем, выбран ли этот репетитор в данный момент
...
function openOrderModal(id, type) {
    ...
    // В функции openOrderModal(id, type)
    const durationInput = document.getElementById('form-duration');
    const durationUnit = document.getElementById('duration-unit');

    if (type === 'course') {
    // Для курса показываем НЕДЕЛИ
        durationUnit.textContent = 'недель';
        durationInput.value = currentItem.total_length; // Берем кол-во недель из данных курса
        durationInput.readOnly = true; // Запрещаем менять, если это курс
    } else {
    // Для репетитора показываем ЧАСЫ
        durationUnit.textContent = 'часов';
        durationInput.value = 1; // По умолчанию 1 час
        durationInput.readOnly = false; // Разрешаем ввод
    }
...
function handleTutorClick(id) {
    // Если нажали на уже выбранного репетитора — отменяем выбор
